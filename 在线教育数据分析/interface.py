...
con2 = create_engine('mysql+pymysql://root:QaQ111.0@localhost:3306/Level_7_model_1?charset=utf8')

with open(os.path.join('sql', 'daily_board.sql'), 'r') as f:
    sql['daily_board'] = f.read()

with open(os.path.join('sql', 'weekly_board.sql'), 'r') as f:
    sql['weekly_board'] = f.read()

with open(os.path.join('sql', 'daily_chart.sql'), 'r') as f:
    sql['daily_chart'] = f.read()

with open(os.path.join('sql', 'weekly_chart.sql'), 'r') as f:
...
def daily_interface():
    ...
    # 读数据
    df_board = pd.read_sql(sql['daily_board'], con=con2)
    df_chart = pd.read_sql(sql['daily_chart'], con=con2)

    # Nav
    date = df_board.loc[0, 'date'].strftime('%Y-%m-%d')
    weekday = weekday_transfer[df_board.loc[0, 'date'].weekday()]

    rlt_data['nav'] = {'date': date, 'weekday': weekday}
    rlt_data['board1'], rlt_data['board2'] = get_board(df_board)
    rlt_data['chart1'], rlt_data['chart2'] = get_chart(df_chart)

    # JSON
    return json.dumps(rlt_data)
...
def weekly_interface():
    ...
    # 读数据
    df_board = pd.read_sql(sql['weekly_board'], con=con2)
    df_chart = pd.read_sql(sql['weekly_chart'], con=con2)
    ...
    rlt_data['nav'] = {'date': '%s至%s' % (date_start, date_end), 'weekday': weekday}
    rlt_data['board1'], rlt_data['board2'] = get_board(df_board)
    rlt_data['chart1'], rlt_data['chart2'] = get_chart(df_chart)
